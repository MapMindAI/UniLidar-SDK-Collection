...
DEFINE_double(plane_inlier_threshold_m, 0.2,
              "Point-to-plane distance threshold used for plane extraction and "
              "assignment.");
...
DEFINE_string(range_model_candidates, "quadratic",
...
DEFINE_double(calibration_range_step_m, 0.01,
              "Initial step size for range correction coefficients.");
...
DEFINE_double(calibration_alpha_step_rad, 0.0005,
              "Initial step size for the theta-based alpha correction coefficients.");
DEFINE_bool(optimize_alpha_theta_coefficients, true,
            "Optimize linear theta-based alpha correction coefficients.");
...
calibration::CalibrationOptimizationConfig MakeCalibrationOptimizationConfig(
  ...
  config.optimize_alpha_theta_coefficients = FLAGS_optimize_alpha_theta_coefficients;
...
calibration::ViewerConfig MakeViewerConfig() {
  ...
  config.initial_calibration_parameters.range_coefficients_m = {0.0f, 0.0f, 0.0f};
  config.initial_calibration_parameters.alpha_theta_coefficients_rad = {0.0f, 0.0f};
  return config;
}
...
int Run(int argc, char** argv) {
  ...
  if (FLAGS_merge_beginning_frames > 0) {
    merged_beginning_frame = std::make_unique<calibration::ReplayFrame>(
    ...
    if (!planes.empty()) {
      ...
      if (calibration_config.enabled) {
        const calibration::CalibrationSolution solution =
            calibration::OptimizeCalibration(*merged_beginning_frame, planes, calibration_config);
        if (std::isfinite(solution.objective)) {
          LOG(INFO) << "Selected calibration model=" << solution.model_name << " coeff_m="
                    << calibration::VectorSummary(solution.parameters.range_coefficients_m)
                    << " alpha_theta_coeff_rad="
                    << calibration::VectorSummary(solution.parameters.alpha_theta_coefficients_rad)
          ...
          calibration::ViewerConfig viewer_config = MakeViewerConfig();
          viewer_config.initial_calibration_parameters = solution.parameters;
          for (calibration::ReplayFrame& frame : frames) {
            calibration::RebuildFramePoints(active_calibration, &frame);
          }
          calibration::RebuildFramePoints(active_calibration, merged_beginning_frame.get());
      ...
      LOG(WARNING) << "No planes extracted from merged cloud. Calibration "
