...
DEFINE_int32(max_planes, 4, "Maximum number of planes to extract from the merged cloud.");
DEFINE_double(plane_inlier_threshold_m, 0.05,
...
DEFINE_bool(optimize_calibration, false, "Optimize calibration against extracted planes.");
...
PlaneExtractionConfig MakePlaneExtractionConfig() {
...
CalibrationOptimizationConfig MakeCalibrationOptimizationConfig(int min_required_assigned_points) {
  CalibrationOptimizationConfig config;
...
int Run(int argc, char** argv) {
  ...
  std::vector<ReplayFrame> frames =
      BuildReplayFrames(packets, FLAGS_accumulate_rings, FLAGS_min_range_m, FLAGS_max_range_m);
  ...
  UniLidarCalibration active_calibration;
  active_calibration.enabled = false;
  for (ReplayFrame& frame : frames) {
    RebuildFramePoints(active_calibration, &frame);
  }

  std::unique_ptr<ReplayFrame> merged_beginning_frame;
  std::vector<PlaneModel> planes;
  const PlaneExtractionConfig plane_config = MakePlaneExtractionConfig();
  const CalibrationOptimizationConfig calibration_config =
  ...
  if (FLAGS_merge_beginning_frames > 0) {
    ...
    RebuildFramePoints(active_calibration, merged_beginning_frame.get());
    ...
    planes = DetectPlanes(*merged_beginning_frame, plane_config);
    LogPlaneSummary(planes);

    if (!planes.empty()) {
      const ResidualSummary baseline =
          EvaluateResiduals(*merged_beginning_frame, planes, calibration_config.assignment_threshold_m);
      ...
      if (calibration_config.enabled) {
        const CalibrationSolution solution = OptimizeCalibration(
        ...
        if (std::isfinite(solution.objective)) {
          LOG(INFO) << "Selected calibration model=" << solution.model_name
                    << " coeff_m=" << VectorSummary(solution.range_coefficients_m)
                    << " alpha_offsets_rad=" << VectorSummary(solution.delta_alpha_min)
