...
void RunViewer(const std::vector<ReplayFrame>& frames, const ReplayFrame* merged_beginning_frame,
  ...
  pangolin::Var<double> ui_range_c0("menu.Range c0", editable_parameters.range_coefficients_m[0],
                                    -0.5, 0.5, false);
  pangolin::Var<double> ui_range_c1("menu.Range c1",
                                    editable_parameters.range_coefficients_m.size() > 1
                                        ? editable_parameters.range_coefficients_m[1]
                                        : 0.0,
                                    -0.5, 0.5, false);
  pangolin::Var<double> ui_range_c2("menu.Range c2",
                                    editable_parameters.range_coefficients_m.size() > 2
                                        ? editable_parameters.range_coefficients_m[2]
                                        : 0.0,
                                    -0.5, 0.5, false);
  pangolin::Var<double> ui_alpha_t0(
      "menu.Alpha t0", editable_parameters.alpha_theta_coefficients_rad[0], -0.02, 0.02, false);
  ...
  pangolin::Var<int> ui_merged_points(
      "menu.Merged Points",
      editable_merged_frame == nullptr ? 0 : static_cast<int>(editable_merged_frame->points.size()),
      0, 0, false);
  ...
  while (!pangolin::ShouldQuit()) {
    ...
    bool parameter_notchange =
        (range_c0 == ui_range_c0 && range_c1 == ui_range_c1 && range_c2 == ui_range_c2 &&
    ...
    if ((!parameter_notchange || pangolin::Pushed(ui_refresh_merged)) &&
        editable_merged_frame != nullptr) {
      ...
      editable_parameters.alpha_theta_coefficients_rad = {static_cast<float>(ui_alpha_t0),
                                                          static_cast<float>(ui_alpha_t1),
                                                          static_cast<float>(ui_alpha_t2)};
